...
__attribute__((naked, noinline)) void HandleRegisters()
{
    ...
    (
        ...
        "bl        ._ZN4hook7handlerER13CallContext_t;"
        "nop;"
...
namespace hook
{
    ...
    void start(uintptr_t location, size_t entryCount)
    {
        ...
        _sys_memset(s_hookStack, 0, sizeof(HookEntry_t) * entryCount);
    ...
    {
        uintptr_t callOrigin = ctx.lr - 4;

        for (int i = 0; i < s_hookStackSize; i++)
        {
            if (s_hookStack[i].address != callOrigin)
                continue;
    ...
    void copy(uintptr_t address, uint32_t* instructions, size_t instructionCount)
    {
        for (int i = 0; i < instructionCount; i++)
        {
            reinterpret_cast<uint32_t*>(address)[i] = instructions[i];
        }
    }
